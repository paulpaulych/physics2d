...
internal fun Segment.belongsTo(line: Line): Boolean {
    return p1.belongsTo(line) && p2.belongsTo(line)
}

//TODO: переписать как-нибудь более адекватно
internal fun Vec.belongsTo(segment: Segment): Boolean {
    val (p1, p2) = segment
    if(p1.x.eq(p2.x)) {
...
private fun intersectionPts(a: Segment, b: Segment): Set<Vec> {
